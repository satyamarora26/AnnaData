...
def test_longer_product_name_cannot_match_registered_product_prefix():
    answer = "Product B Plus 500 ml/ha is suitable. Remove affected leaves."
    gathered = {
        "_guard_context": {"intent": "disease_pest"},
        "doses": "Registered pesticide uses: Product B, dose 500 ml/ha.",
        "_dose_records": [
    ...
    assert changed
    assert "Product B Plus" not in cleaned
    assert "Remove affected leaves" in cleaned
