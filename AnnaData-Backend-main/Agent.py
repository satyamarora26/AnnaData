...
def get_farming_advice(location, state, crop, gathered, farmer_query,
    ...
    - Apply evidence scope before authority: use a passage only when its declared state and crop scope applies. A matching scoped EXTENSION passage can be more useful than unscoped central material for local practice; among equally applicable passages, prefer OFFICIAL material. REFERENCE material never authorizes a scheme amount, deadline, MSP, or fertiliser quantity.
